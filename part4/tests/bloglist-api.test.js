...
beforeEach(async () => {
  ...
  const user = new User({ username: helper.user.username, passwordHash, name: helper.user.name })
  await user.save()
  const secondPasswordHash = await bcrypt.hash(helper.secondUser.password, 10)
  const secondUser = new User({ username: helper.secondUser.username, secondPasswordHash, name: helper.secondUser.name })
...
describe('post methods', () => {
  test('blog post can be added to the database', async () => {
    const login = { username: helper.user.username, password: helper.user.password }
